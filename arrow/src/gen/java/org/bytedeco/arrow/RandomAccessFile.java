...
public class RandomAccessFile extends InputStream {
  ...
  ///
  public static native @ByVal InputStreamResult GetStream(
        @SharedPtr @Cast({"", "std::shared_ptr<arrow::io::RandomAccessFile>"}) RandomAccessFile file, @Cast("int64_t") long file_offset, @Cast("int64_t") long nbytes);
   ...
   * 
   *  @param ranges [in] The ranges to read
   *  @return A future that will complete with the data from the requested range is
   *  available */
  public native @StdVector BufferFuture ReadManyAsync(
        @Const @ByRef IOContext arg0, @StdVector ReadRange ranges);

  /** EXPERIMENTAL: Explicit multi-read, using the file's IOContext. */
  
  ///
  public native @StdVector BufferFuture ReadManyAsync(
        @StdVector ReadRange ranges);
