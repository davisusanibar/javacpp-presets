...
@Name("std::vector<std::shared_ptr<arrow::Field> >") @Properties(inherit = org.bytedeco.arrow.presets.arrow.class)
public class FieldVector extends Pointer {
    static { Loader.load(); }
    ...
    public native @Name("operator =") @ByRef FieldVector put(@ByRef FieldVector x);
    ...
    public Field[] get() {
        Field[] array = new Field[size() < Integer.MAX_VALUE ? (int)size() : Integer.MAX_VALUE];
    ...
    public Field pop_back() {
        long size = size();
        Field value = get(size - 1);
        resize(size - 1);
        return value;
    }
    public FieldVector push_back(Field value) {
        long size = size();
        resize(size + 1);
        return put(size, value);
    }
    public FieldVector put(Field value) {
        if (size() != 1) { resize(1); }
        return put(0, value);
    }
    public FieldVector put(Field ... array) {
